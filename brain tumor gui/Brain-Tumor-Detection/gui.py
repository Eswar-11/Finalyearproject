...
class Gui:
    ...
    def __init__(self):
        ...
        RB1 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="Detect Tumor using cnn", variable=self.val,
                                  value=1, background = "light blue", font=12,activeforeground="green", command=self.check)
        RB1.place(x=250, y=250)
                                  
        RB2 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="Detect tumor using watershed",
                                  variable=self.val, background = "light blue", font=12,activeforeground="green",value=3, command=self.check)
        RB2.place(x=250, y=300)


        RB3 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="View Tumor Region",
                                  variable=self.val, value=2, background = "light blue", font=12,activeforeground="green",command=self.check)
        RB3.place(x=250, y=350)

        

        browseBtn = tkinter.Button(self.FirstFrame.getFrames(), fg="black",text="Browse", width=22, background = "#90E0EF", font=15,activeforeground="green",command=self.browseWindow)
        ...
        if (self.val.get() == 3):
            ...
            if res>0.5:
                ...
                if res>0.8:
                    d="3"
            if res>0.5:
                resLabel = tkinter.Label(self.FirstFrame.getFrames(), text="Tumor Detected,Stage ", height=1, width=20)
            ...
            if res2>0.5:
                ...
                if res2>0.8:
                    d="3"
            if res==1:
                resLabel = tkinter.Label(self.FirstFrame.getFrames(), text="Tumor Detected,Stage"+d, height=1, width=20)
                resLabel.configure(background="White", font=("Comic Sans MS", 16, "bold"), fg="red")
            else:
                resLabel = tkinter.Label(self.FirstFrame.getFrames(), text="No Tumor", height=1, width=20)
                resLabel.configure(background="White", font=("Comic Sans MS", 16, "bold"), fg="green")
